...
//		System.out.println(s.length());
	...
	public Result updateBrand(int brandId, String name, String logo, String keywords, 
			String description, String url) {
		Result result=new Result();
		Brand checkBrand1=brandDao.findById(brandId);
		if(checkBrand1==null){
			result.setStatus(1);
			result.setMsg("没有此品牌");
			return result;
		}
		Brand checkBrand2=brandDao.findByName(name);
		if(checkBrand2!=null && !name.equals(checkBrand1.getName())){
			result.setStatus(1);
			result.setMsg("此品牌名称已经存在");
			return result;
		}
		Brand brand=new Brand();
