...
$(document).ready(function(){
		       		...
		       		goods.fetch({
		       		 success:function(model,response){
		         		...
		         		//console.log("商品编号：" +model.get("sn")+"   商品名称："+ model.get("name")+"    商品价格："+model.get("price"));
		       		   ...
		       		   /*  $("#editGoods .introArea").append("<textarea id='intro'></textarea>");*/
	                     var editor1 = KindEditor.create('textarea[id="intro"]',{
	                    	 themeType: "simple",
	                         uploadJson: 'http://localhost:8080/MobileShop/file2/img/upload',
	                     ...
	                     afterCreate: function () {
	                         this.sync();// 同步数据后可以直接取得textarea的value 后面有console.log("文本框内的数据"+$("#intro").val().trim());
	                     },
	                     afterBlur: function () {
	                         this.sync();//同步数据
	                     },
	                     });//这句在后面即可
	                      // editor.html(model.get("intro"));
	                     
		       		     //绑定事件到提交按钮上
		       		     $("a#editGoodsBtn").on('click',function(){//a#editGoodsBtn这个按钮，不在这个 View的this.$el范围内，所以不能用上面的events来绑定
		       		    	 //提取数据
		       		    	console.log("文本框内的数据"+$("#intro").val().trim());
  ...
  //点击上一页、下一页时翻页
	$("#pageGro .pageDown,#pageGro .pageUp").on("click",function(){
		var pageId =  parseInt($("#pageGro li.on").html());
		var path = "http://localhost:8080/MobileShop";
		var url = path+"/goods/enable/page/"+pageId;
        onLineGoodsList.fetch( {reset:true,
                url:url,//也可以在fetch时重构url
