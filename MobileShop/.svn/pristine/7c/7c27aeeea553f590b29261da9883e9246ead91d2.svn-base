...
//		System.out.println("service");
	...
	//根据id更新会员基本信息
	public Result updateMember(int memberId, String uname, String email,
			int sex, String mobile) {
		Result result=new Result();
		//判断是否存在此会员
		Member checkMember1=memberDao.findById(memberId);
		if(checkMember1==null){
			result.setStatus(1);
		...
		if(!"".equals(uname)){
			...
			map.put("uname", uname);
			Member checkMember2=memberDao.dynamicFind(map);
	...
	public Result resetPwd(String email) {
		Result result=new Result();
		Member checkMember=memberDao.findByEmail(email);
		if(checkMember==null){
			result.setStatus(1);
			result.setMsg("没有此会员，请检查邮箱是否正确");
			return result;
		}
		int memberId=checkMember.getMember_id();
		//构建动态密码
	...
	public Result updateImage(int memberId, String img, String url) {
		...
		String image=s[0]+"_member."+s[1];
		MSUtil.resizeImage(img, image, 64, 64);
		String path=MSUtil.getPath();
		String memberImage=image.replace(path, url);
		Member member=new Member();
		member.setMember_id(memberId);
		//将资源路径中的\替换为/
		member.setImage(memberImage.replace("\\", "/"));
		memberDao.dynamicUpdate(member);
		//将原图删除
		MSUtil.deleteFile(img);
		result.setStatus(0);
		result.setMsg("设置会员头像成功");
		return result;
	}
