...
public class ShipServiceImpl implements ShipService {
	...
	public Result updateShip(int shipId, String name, String code) {
		Result result=new Result();
		Ship checkShip=shipDao.findById(shipId);
		Ship findShip1=new Ship();
		findShip1.setName(name);
		Ship checkShip1=shipDao.dynamicFind(findShip1);
		if(checkShip1!=null && !name.equals(checkShip.getName())){
			result.setStatus(1);
			result.setMsg("已经存在此货运公司");
			return result;
		}
		Ship findShip2=new Ship();
		findShip2.setCode(code);
		Ship checkShip2=shipDao.dynamicFind(findShip2);
		if(checkShip2!=null && !code.equals(checkShip.getCode())){
			result.setStatus(1);
			result.setMsg("已经存在此代码");
			return result;
		}
		Ship ship=new Ship();
